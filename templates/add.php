...
<form class="form"  action="add.php" enctype="multipart/form-data" method="post" autocomplete="off">
    ...
    <div class="form__row">
        ...
        <?= (empty($_POST['deadline'])&&(!empty($_POST))) ? '<p class ="form__message">Выберите корректную дату!</p>' : '' ?>
        <input class="form__input form__input--date <?= (empty($_POST['deadline'])&&(!empty($_POST))) ? 'form__input--error' : ''?>" type="text" name="date" id="date" value="" placeholder="Введите дату в формате ГГГГ-ММ-ДД"
               value ="<?= !empty($_POST)? $_POST['deadline'] : '' ?>">
    </div>
